...
const collectItems = (results, categoryId) => {
    if (categoryId === 1) {
        return results.map(item => ({ id: item.url, title: item.name, src: getImages(item.url), url: item.url, birth: item.birth_year, gender: item.gender, height: item.height, mass: item.mass }));
    }
